...
export function WelcomeWizard({ onAccountCreated }: IWelcomeWizardProps): React.JSX.Element {
    ...
    if (step === 'enter-details' && selectedType && TypeGuards.isLoanAccount(selectedType)) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-8 animate-in fade-in duration-300 cursor-default">
                <LoanDetailsForm
                    type={selectedType}
                    onBack={handleBack}
    ...
    if (step === 'enter-details' && selectedType) {
        ...
        const theming = TYPE_THEMING[selectedType] || {
            accentBg: 'bg-primary',
        ...
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-8 animate-in fade-in duration-300 cursor-default">
                <Card
                    ...
                    className="w-full max-w-2xl p-8 border-border/50"
                >
    ...
    // Select Type Step
    return (
        <div className="flex-1 flex flex-col items-center justify-center p-8 cursor-default">
            <div className="w-full max-w-4xl py-12">
                <div className="text-center mb-10">
                    <Link
                        href="/"
                        className="flex items-center justify-center gap-2 mb-3 group hover:opacity-80 transition-opacity"
                    >
                    ...
                    <p className="text-sm text-muted-foreground max-w-xl mx-auto">
                        Let&apos;s get started by creating your first account. Choose the type that
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8 max-w-[600px] mx-auto">
                    {PRIMARY_TYPES.map((option: IAccountTypeOption): React.JSX.Element => {
                        ...
                        const theming = TYPE_THEMING[option.type] || {
                            accentBg: 'bg-primary',
                            borderHover: 'border-primary/30',
                            iconText: 'text-primary',
                            iconBg: 'bg-primary/10',
                        ...
                        return (
                            <Card
                                ...
                                interactive
                                accentColor={theming.accentBg}
                                className={cn('p-6', theming.borderHover)}
                            >
                ...
                <div className="flex flex-col items-center">
                    ...
                    {showLoans && (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 w-full max-w-[600px] mx-auto animate-in slide-in-from-top-4 duration-300 fill-mode-forwards">
                            {LOAN_TYPES.map(
                                (option: IAccountTypeOption, idx: number): React.JSX.Element => {
                                    const Icon = option.icon;
                                    const isLastOdd =
                                        idx === LOAN_TYPES.length - 1 &&
                                        LOAN_TYPES.length % 2 !== 0;
