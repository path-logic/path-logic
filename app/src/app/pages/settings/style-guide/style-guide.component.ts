...
export class StyleGuideComponent {
    ...
    readonly semanticSwatches: Array<IColorSwatch> = [
        { token: 'accent-violet', hex: '#8B5CF6', usage: 'Primary branding, active states, focus rings' },
    ...
    readonly fontRoles: Array<IFontRole> = [
        { role: 'Primary / Headings', family: 'Outfit',           weights: '500, 600, 700, 900', usage: 'Dashboard figures, section titles, marketing headers' },
        { role: 'Secondary / UI / Data', family: 'Inter',         weights: '400, 500',           usage: 'Row data, forms, navigation, microcopy' },
        { role: 'Tertiary / Code',    family: 'JetBrains Mono',   weights: '400',                usage: 'Transaction IDs, encryption keys, raw JSON exports' }
    ];
    ...
    readonly layoutRules: Array<ILayoutRule> = [
        ...
        { name: 'Data Density',       detail: 'Table rows: 24px–32px. Minimize padding. Maximize visible data.' },
        { name: 'Border Radius',      detail: 'Max 4px (rounded-sm). No pill buttons. No excessive rounding.' },
        { name: 'Shadows',            detail: 'Minimal. Prefer border-based elevation over box-shadow.' },
        { name: 'Animation Speed',    detail: 'All hover/color transitions < 100ms. Use CSS transition: 80ms ease-out.' },
        { name: 'Keyboard Navigation',detail: 'CMD+K palette, Arrow keys for rows, Enter to edit, Escape to cancel.' }
    ];

    // ── Accessibility Rules ───────────────────────────────────────────────

    readonly a11yRules: Array<ILayoutRule> = [
        { name: 'Color Independence', detail: 'Never rely on color alone. Positive numbers get + prefix. Negative numbers get − or parentheses.' },
        { name: 'Contrast Ratios',    detail: 'WCAG 2.1 AA minimum: 4.5:1 normal text, 3:1 large text. Verify in both themes.' },
        { name: 'Focus Rings',        detail: 'Mandatory: 2px solid #8B5CF6 offset outline on all focusable elements. Never outline: none.' },
        { name: 'ARIA Labels',        detail: 'All icon-only buttons require aria-label. Tables must have role, aria-sort, and column headers.' },
        { name: 'Reduced Motion',     detail: 'Respect prefers-reduced-motion: reduce. Disable all non-essential animations.' },
        { name: 'Minimum Font Size',  detail: '13px for data tables. 14px for form labels and UI text.' }
    ];
}
